...
def get_df(filename):
    ...
    return df2, df3, df_print
df, df_names, df_print = get_df(sys.argv[1])
if (len(sys.argv) == 3):
    print("reading second array")
    df2, df2_names, _ = get_df(sys.argv[2])
...
for i in range(0, frame_len):
   if (i in keep_list):
      ...
      print('Dropping ', i)
      df2 = df2.drop([i+1], axis=0)
      df_print = df_print.drop([i+1], axis=0)
      
df_print.to_csv("kernel_signatures_rep.csv")
print(len(df2))
